...
@section('content')
                    ...
                    <a class="btn btn-success" href="javascript:void(0)" id="createNewDiagnostic">{{__('messages.Create New Diagnostic')}}</a>
...
<script type="text/javascript">
    $(function () {
        $('#page-tage').html('{{__("messages.Diagnostic")}}');
      /*------------------------------------------
       --------------------------------------------
       Pass Header Token
       --------------------------------------------
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('#createNewDiagnostic').click(function () {
          $('#saveBtn').val("create-diagnostic");
          $('#diagnostic_id').val('');
          $('#adminForm').trigger("reset");
          $('#modelHeading').html('{{__("messages.Create New Diagnostic")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.editDiagnostic', function () {
        var diagnostic_id = $(this).data('id');
        $.get("{{ route('diagnostic.index') }}" +'/' + diagnostic_id +'/edit', function (data) {
            $('#modelHeading').html("{{ __('messages.Edit') }}");
            $('#saveBtn').val("edit-diagnostic");
            $('#ajaxModel').modal('show');
            $('#diagnostic_id').val(data.id);
      ...
      $('body').on('click', '.deleteDiagnostic', function () {
          ...
          if(confirm("{{__('messages.Are You sure want to delete !')}}")){
            ...
            $.ajax({
              type: "DELETE",
              url: "{{ route('diagnostic.store') }}"+'/'+diagnostic_id,
              success: function (data) {
                  table.draw();
              },
              error: function (data) {
