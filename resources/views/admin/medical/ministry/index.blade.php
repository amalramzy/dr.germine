...
@section('content')
                    <a class="btn btn-success" href="javascript:void(0)" id="createNewMinistry"> {{__('messages.Create New Ministry')}}</a>
...
<script type="text/javascript">
    $(function () {
        $('#page-tage').html('{{__("messages.Ministry Vaccination")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('#createNewMinistry').click(function () {
          $('#saveBtn').val("create-ministry");
          $('#ministry_id').val('');
          $('#adminForm').trigger("reset");
          $('#modelHeading').html('{{__("messages.Create New Ministry")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.editMinistry', function () {
        var ministry_id = $(this).data('id');
        $.get("{{ route('ministry.index') }}" +'/' + ministry_id +'/edit', function (data) {
            $('#modelHeading').html("{{ __('messages.Edit') }}");
            $('#saveBtn').val("edit-ministry");
            $('#ajaxModel').modal('show');
            $('#ministry_id').val(data.id);
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.deleteMinistry', function () {
          ...
          if(confirm("{{__('messages.Are You sure want to delete !')}}")){
            ...
            $.ajax({
              type: "DELETE",
              url: "{{ route('ministry.store') }}"+'/'+ministry_id,
              success: function (data) {
                  table.draw();
              },
              error: function (data) {
