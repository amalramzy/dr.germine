...
@section('content')
                    <a class="btn btn-success" href="javascript:void(0)" id="createNewMedicine"> {{__('messages.Create New Medicine')}}</a>
...
<script type="text/javascript">
    $(function () {
        $('#page-tage').html('{{__("messages.Medicine")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('#createNewMedicine').click(function () {
          $('#saveBtn').val("create-medicine");
          $('#medicine_id').val('');
          $('#adminForm').trigger("reset");
          $('#modelHeading').html('{{__("messages.Create New Medicine")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.editMedicine', function () {
        var medicine_id = $(this).data('id');
        $.get("{{ route('medicine.index') }}" +'/' + medicine_id +'/edit', function (data) {
            $('#modelHeading').html("{{ __('messages.Edit') }}");
            $('#saveBtn').val("edit-medicine");
            $('#ajaxModel').modal('show');
      ...
      $('body').on('click', '.deleteMedicine', function () {
          ...
          if(confirm("{{__('messages.Are You sure want to delete !')}}")){
            ...
            $.ajax({
              type: "DELETE",
              url: "{{ route('medicine.store') }}"+'/'+medicine_id,
              success: function (data) {
                  table.draw();
              },
              error: function (data) {
