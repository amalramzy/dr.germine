...
@section('content')
                    <a class="btn btn-success" href="javascript:void(0)" id="createNewMedical"> {{__('messages.Create New Medical')}}</a>
...
<script type="text/javascript">
    $(function () {
        $('#page-tage').html('{{__("messages.Medical Test")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('#createNewMedical').click(function () {
          $('#saveBtn').val("create-medical");
          $('#medical_id').val('');
          $('#adminForm').trigger("reset");
          $('#modelHeading').html('{{__("messages.Create New Medical")}}');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.editMedical', function () {
        var medical_id = $(this).data('id');
        $.get("{{ route('medicaltest.index') }}" +'/' + medical_id +'/edit', function (data) {
            $('#modelHeading').html("{{ __('messages.Edit') }}");
            $('#saveBtn').val("edit-medical");
            $('#ajaxModel').modal('show');
      ...
      --------------------------------------------
      --------------------------------------------*/
      $('body').on('click', '.deleteMedical', function () {
          ...
          if(confirm("{{__('messages.Are You sure want to delete !')}}")){
            ...
            $.ajax({
              type: "DELETE",
              url: "{{ route('medicaltest.store') }}"+'/'+medical_id,
              success: function (data) {
                  table.draw();
              },
              error: function (data) {
